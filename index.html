...
input[type="checkbox"]{height:unset!important}

body{background:#525659;display:inline-flex;width:100vw;min-height:100vh;margin:25px 0}
#paper{background-color:#fffdf1;background-image:url("noisy.png");min-width:900px;width:55%;padding:50px 30px;margin:auto}
...
.verysmall{width:5%}
.verylarge{width:90%}
.border1,.border2{border:6px solid transparent;border-image:url("border.svg") 20% stretch;z-index:0}

input,textarea{background:#fdfdfd;width:100%;text-align:center;float:left;overflow:auto;padding:7px}
select{-webkit-appearance:none;width:100%;text-align-last:center;height:100%}
...
#cond_overlay,#habi_overlay,#equip_overlay,#spell_overlay{position:fixed;width:100vw;height:100%;display:flex;background:rgba(0,0,0,.6);top:0;overflow:auto}
...
#habis,#habis_list,#anotat,#anot,#spells,#spells_list,#equips,#equips_list{overflow:overlay}
...
#roller{position:fixed;bottom:0;right:0;width:15%;min-height:400px;background:#bdbcbc;overflow:hidden}
...
.dice{background:#fff url("dice.png") center/contain no-repeat;cursor:pointer;margin:1% 0}
#weapons .dice{margin:1% 10px 1% 0}
#pgdivider{width:calc(100% + 60px);height:25px;left:-30px;margin:50px 0;background:#525659}
  ...
  <section id="header">
   <div class="row">
    <div class="name border1">
     ...
     <div class="name_rmv"><div><button class="rmv" onclick="rmv_char()">x</button></div></div>
    </div>
    <div class="player border2"><input placeholder="Jogador" name="bplayer" onchange="change(this)"></div>
    <img class="logo" src="logo.png" alt="Tormenta 20" ondblclick="svg_esc()" onmousedown="svg_shape_color(event)"/>
   </div>
   <div class="row">
   ...
   <div class="row">
    <div class="left">
     ...
     <div id="subheader" class="row">
      ...
      <div class="deslc border2"><input type="number" placeholder="Deslocamento" name="bdeslc" onchange="change(this)"></div>
    ...
    <div id="spells" class="pmpv border2">
     ...
     <div class="row">
      <div class="container ms">
       <div class="half df"><div class="ma">Atributo chave:</div><div class="ma"><select name="shab" class="select" onchange="change_spell(this)"><option value="3">int</option><option value="4">sab</option><option value="5">car</option></select></div></div>
...
<div id="roller">
<div id="results" onmouseleave="print_out(0)">
<p class="bold">Exemplos de rolagens:</p>
</div>
<input id="dice" onchange="diceroll(this.value);this.value=''" onkeypress="return /[0-9#d+-kehl]/i.test(event.key)">
</div>
...
<div id="equip_overlay" class="invisible" onclick="hide(event,this)"><div id="equip">
 ...
 <div class="row"><div class="container">
  <div class="half">
   ...
   <select class="select" onchange="check_other(this,1)">
    <option value="Tipo de equipamento" selected="" disabled="">Tipo de equipamento</option>
    <option value="Arma">Arma</option>
    <option value="Armadura">Armadura</option>
    <option value="Escudo">Escudo</option>
    <option value="Item geral">Item geral</option>
   ...
   <input type="text" placeholder="Peso: 1 kg">
  </div>
  <div class="half">
...
<div id="spell_overlay" class="invisible" onclick="hide(event,this)"><div id="spell">
 ...
 <div class="row"><div class="container">
  <div class="half"><input type="text" placeholder="T&iacute;tulo"></div>
  <div class="half"></div>
 </div></div>
 <div class="row"><div class="container">
  <div class="half">
   ...
   <select class="select ssw30" onchange="check_other(this,2)">
    <option value='C&iacute;rculo' selected disabled>C&iacute;rculo</option>
 ...
 <div class="row"><div class="container">
  ...
  <div class="half">
   <select class="select" onchange="check_other(this,0)">
    <option value='Alcance' selected disabled>Alcance</option>
 ...
 <div class="row"><div class="container">
  <div class="half"><input type="text" placeholder="&Aacute;rea/alvo"></div>
  <div class="half">
   <select class="select" onchange="check_other(this,0)">
 ...
 <div class="row"><div class="container">
  <div class="half"><input type="text" placeholder="Resist&ecirc;ncia"></div>
  <div class="half"><input type="text" placeholder="Como conseguiu (origem e n&iacute;vel)"></div>
 </div></div>
 ...
 <div style="margin:20px 0 0 auto;width:fit-content">
  <button id="btn1" class="btn btn1" onclick="add_generic(this,spells);add_spell(spells.length-1)">Salvar</button>
...
var chars = [];
var header, pmpv, defense, habs, weapons, spells, equips, features, skills, oficios, active_conditions, effects
...
function getIndicesOf(str){
 ...
 for(i=0;i<keys.length;i++){
  var startIndex = 0;
  while ((index = str.indexOf(keys[i],startIndex)) > -1){
   indices.push([index,keys[i].length]);
   startIndex = index + keys[i].length;
  }
 }
 return indices;
}
...
function print_out(print){
 ...
 setTimeout(function(){demo.scrollTop=demo.scrollHeight},300);
}

function roll_weap(el){
 el = el.parentNode;
 print_out('Ataque com '+el.children[1].children[0].value);
 diceroll(el.children[2].children[0].value);
 console.log('roll = '+roll[0]);
 if(roll[0]>=JSON.parse(el.children[4].children[0].value)){
  print_out('Dano crítico!');
  diceroll('2#'+el.children[3].children[0].value);
 }else{
  diceroll(el.children[3].children[0].value);
 }
 print_out('<br>');
}
...
function roll_skill(el,ski){
 el = el.parentNode;
 var per = el.children[3].innerHTML;
 if(per=='-'){}
 else{
  if(ski){print_out('Teste de '+el.children[2].innerHTML.replace(/[*‡]/g,''))}
  else{print_out('Teste de '+el.children[2].children[0].value)}

  if(per.includes('-')){diceroll('d20'+per)}
  else{diceroll('d20+'+per)}
  print_out('<br>');
 }
}

print_out('d20+2');diceroll('d20+2');
print_out('2d20-2');diceroll('2d20-2');
print_out('2#d12+1');diceroll('2#d12+1');
print_out('2d10+2d8-2');diceroll('2d10+2d8-2');
print_out('3d8! (explosão)');diceroll('3d8!');
print_out('3d8!5 (explosão em 5+)');diceroll('3d8!5');
print_out('4d6kh3 (keep higher 3)');diceroll('4d6kh3');
print_out('4d6el (eliminate lower)');diceroll('4d6el');
print_out('5d4!kl2 (keep lower 2)');diceroll('5d4!kl2');
print_out('5d4!3eh2 (eliminate higher 2)');diceroll('5d4!3eh2');
print_out('------');
...
function add_spell(index){
 ...
 var text = '<div class="verylarge"><bg>'+spells[index][0]+'</bg>';
 if(spells[index][1]||spells[index][2]||spells[index][3]||spells[index][9]){text += ' '}
 if(spells[index][1]){text += spells[index][1]}
 if(spells[index][1]&&spells[index][2]){text += ' '}
 if(spells[index][2]){text += spells[index][2]}
 if(spells[index][3]){text += ' <i>'+spells[index][3]+'</i>'}
 if(spells[index][9]){text += ' <sm>('+spells[index][9]+')</sm>'}

 if(spells[index][2]||spells[index][4]||spells[index][5]||spells[index][6]||spells[index][7]||spells[index][8]){text += '<br><sm>'}
 if(spells[index][2]){text += '<span class="nowrap"><b>Custo: </b> '+spells[index][2]+' PM</span>&nbsp;&nbsp;&nbsp;&nbsp;'}
 if(spells[index][4]){text += '<span class="nowrap"><b>Ação:</b> '+spells[index][4]+'</span>&nbsp;&nbsp;&nbsp;&nbsp;'}
 if(spells[index][5]){text += '<span class="nowrap"><b>Alcance:</b> '+spells[index][5]+'</span>&nbsp;&nbsp;&nbsp;&nbsp;'}
 if(spells[index][6]){text += '<span class="nowrap"><b>Área/Alvo:</b> '+spells[index][6]+'</span>&nbsp;&nbsp;&nbsp;&nbsp;'}
 if(spells[index][7]){text += '<span class="nowrap"><b>Duração:</b> '+spells[index][7]+'</span>&nbsp;&nbsp;&nbsp;&nbsp;'}
 if(spells[index][8]){text += '<span class="nowrap"><b>Resistência:</b> '+spells[index][8]+'</span>'}
 if(spells[index][2]||spells[index][4]||spells[index][5]||spells[index][6]||spells[index][7]||spells[index][8]){text += '</sm>'}

 if(spells[index].length>10){text += '<div class="item_triag"></div><div class="item_detail">'}
 if(spells[index][10]){text += '<b>Descrição:</b><br>'+spells[index][10]+'<br>'}
 for(var i=11;i<spells[index].length;i+=2){
  text += '<br>';
  if(spells[index][i]&&spells[index][i+1]){
   if(spells[index][i]>0){text += '<b>+'+spells[index][i]+' PM: '}
   else{text += '<b>Truque: '}
   text += '</b>'+spells[index][i+1];
  }
 }
 if(spells[index].length>10){text += '</div>'}
 text += '</div><div class="verysmall"><button class="rmv" onclick="rmv_item(this,'+index+',spells)">x</button></div>';
...
function set_spells(){
 ...
 select[0].selectedIndex = header['shab']-3;
 change_spell(select[0]);
 for(var i=0;i<spells.length;i++){add_spell(i)}
}
...
function set_char(el){
 if(title){save_data();console.log('saved '+title)}
 title = el.innerHTML;
...
function rmv_char(){
 var k = document.getElementById('name-char').value;
...
function save_data(){
 ...
 if(title){
  ...
  localStorage.setItem(title+'_spells',JSON.stringify(spells));
...
function load_data(){
 ...
 if(load&&localStorage.getItem(title+'_header')!==null){
  ...
  spells = JSON.parse(localStorage.getItem(title+'_spells'));
